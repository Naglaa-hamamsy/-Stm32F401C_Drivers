...
LED_tenuErrorStatus LED_enuSetLedState(LED_PINS_t Cpy_enuLedPin, u8 Cpy_u8State )
{
	...
	static u8 State_Toggle = STATE_LED_ON;
	...
	else if(Cpy_u8State == STATE_LED_TOGGLE)
	{
		...
		State_Toggle ^= 1;
		if (GPIO_enuWritePinValue(LED_CFG[Cpy_enuLedPin].Port , LED_CFG[Cpy_enuLedPin].Pin, LED_CFG[Cpy_enuLedPin].Mode ^ State_Toggle) == GPIO_enuOK)
			{
